...
final class BlockingIterable<T> implements Iterable<T>, Scannable {
	...
	static final class SubscriberIterator<T> implements InnerConsumer<T>, Iterator<T>, Runnable {
		...
		static final AtomicReferenceFieldUpdater<SubscriberIterator, Subscription> S = AtomicReferenceFieldUpdater
			.newUpdater(SubscriberIterator.class, Subscription.class, "s");
		...
		public boolean hasNext() {
			...
			// throw new IllegalStateException("Iterating over a toIterable() / toStream() is blocking, which is not
			// supported in thread " + Thread.currentThread().getName());
			// }
			for (; ; ) {
				boolean d = done;
