...
public class LocalSessionFactoryBuilder extends Configuration {
	 ...
	 *
	 * @param dataSource the JDBC DataSource that the resulting Hibernate
	 *                   SessionFactory should be using (may be {@code null})
	 */
	 ...
	 *                        SessionFactory should be using (may be {@code null})
	 * @param resourceLoader  the ResourceLoader to load application classes from
	 * @param metadataSources the Hibernate MetadataSources service to use (e.g.
	...
	/**
	 * Set the Spring {@link JtaTransactionManager} or the JTA
	 * {@link TransactionManager} to be used with Hibernate, if any. Allows for
	 * using a Spring-managed transaction manager for Hibernate 5's session and
	 * cache synchronization, with the "hibernate.transaction.jta.platform"
	 * automatically set to it.
	 * <p>
	 * A passed-in Spring {@link JtaTransactionManager} needs to contain a JTA
	 * {@link TransactionManager} reference to be usable here, except for the
	 * WebSphere case where we'll automatically set
	 * {@code WebSphereExtendedJtaPlatform} accordingly.
	 * <p>
	 * Note: If this is set, the Hibernate settings should not contain a JTA
	 * platform setting to avoid meaningless double configuration.
	 */
...
//	}
	...
	/**
	 * Set a Hibernate
	 * {@link org.hibernate.resource.beans.container.spi.BeanContainer} for the
...
//	}
	...
	/**
	 * Set the Hibernate {@link RegionFactory} to use for the SessionFactory. Allows
	 * for using a Spring-managed {@code RegionFactory} instance.
	 * <p>
	 * Note: If this is set, the Hibernate settings should not define a cache
	 ...
	 * annotated classes with this {@code Configuration}.
	 *
	...
	/**
	 * Check whether any of the configured entity type filters matches the current
	 * class descriptor contained in the metadata reader.
	 */
	 ...
	 * using the given executor for a parallel initialization phase (e.g. a
	 ...
	 * injection purposes instead of waiting for Hibernate's bootstrapping to
	 * complete. However, note that the first actual call to a
	 * {@code SessionFactory} method will then block until Hibernate's bootstrapping
	 * completed, if not ready by then. For maximum benefit, make sure to avoid
	 * early {@code SessionFactory} calls in init methods of related beans, even for
