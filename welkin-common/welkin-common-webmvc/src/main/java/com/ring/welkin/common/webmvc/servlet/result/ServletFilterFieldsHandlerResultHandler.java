package com.ring.welkin.common.webmvc.servlet.result;
...
public class ServletFilterFieldsHandlerResultHandler implements HandlerMethodReturnValueHandler, BeanPostProcessor {
    ...
    public boolean supportsReturnType(MethodParameter returnType) {
        ...
        Class<?> declaringClass = returnType.getMethod()
                                            .getDeclaringClass();
    ...
    public void handleReturnValue(Object returnValue, MethodParameter returnType, ModelAndViewContainer mavContainer,
        ...
        om.setSerializerFactory(objectMapper.getSerializerFactory());
        FilterFieldsJsonSerializer jsonSerializer = new FilterFieldsJsonSerializer(om);
        // 如果是JsonBody就循环调用
        Arrays.asList(annos).forEach(a -> {
            if (a instanceof JsonBodys) {
                JsonBodys body = (JsonBodys) a;
                Arrays.asList(body.value())
                      .forEach(json -> {
        ...
        if (returnValue instanceof ResponseEntity) {
            ...
            json = jsonSerializer.toJson(responseEntity.getBody());
        } else {
            json = jsonSerializer.toJson(returnValue);
        }
        response.getWriter()
                .write(json);
    }
