...
import com.ring.welkin.common.persistence.mybatis.enhance.TkEntityColumn;
import com.ring.welkin.common.persistence.mybatis.mapper.example.Example.Criteria;
import com.ring.welkin.common.persistence.mybatis.mapper.example.Example.Criterion;
...
public class ExampleSqlHelper {
    ...
    public static String exampleWhereClause(String record) {
        return "<if test=\"_parameter != null\"><where>"
            + "${@com.ring.welkin.common.persistence.mybatis.mapper.example.OGNL@andNotLogicDelete(_parameter)}"
            + "<trim prefixOverrides=\"and|or\">" + loop(0, 9, record) + "</trim></where></if>";
    }
    ...
    public static String exampleHasAtLeastOneCriteriaCheck(String parameterName) {
        StringBuilder sql = new StringBuilder();
        sql.append(
            "<bind name=\"exampleHasAtLeastOneCriteriaCheck\" value=\"@com.ring.welkin.common.persistence.mybatis.mapper.example.OGNL@exampleHasAtLeastOneCriteriaCheck(");
     ...
     * @return
     */
    public static String exampleSelectColumns(Class<?> entityClass, boolean allColumn) {
        StringBuilder sql = new StringBuilder();
        sql.append("<choose>");
        sql.append(
            "<when test=\"@com.ring.welkin.common.persistence.mybatis.mapper.example.OGNL@hasSelectColumns(_parameter)\">");
        ...
        // 不支持指定列的时候查询全部列
        sql.append("<otherwise>");
        sql.append(getAllColumns(entityClass, allColumn));
        sql.append("</otherwise>");
        sql.append("</choose>");
        return sql.toString();
    }
     ...
     * @return
     */
    public static String getAllColumns(Class<?> entityClass, boolean allColumn) {
        Set<EntityColumn> columnSet = EntityHelper.getColumns(entityClass);
        StringBuilder sql = new StringBuilder();
        for (EntityColumn entityColumn : columnSet) {
            // 如果有ColumnQueryIgnore注解并且设置根据example查询忽略的字段，这里跳过
            if (!allColumn && entityColumn instanceof TkEntityColumn) {
                TkEntityColumn column = (TkEntityColumn) entityColumn;
                if (column.isIgnoreByQuery()) {
                    continue;
                }
            }
    ...
    public static String exampleCountColumn(Class<?> entityClass) {
        ...
        sql.append("<choose>");
        sql.append(
            "<when test=\"@com.ring.welkin.common.persistence.mybatis.mapper.example.OGNL@hasCountColumn(_parameter)\">");
    ...
    public static String exampleForUpdate() {
        StringBuilder sql = new StringBuilder();
        sql.append(
            "<if test=\"@com.ring.welkin.common.persistence.mybatis.mapper.example.OGNL@hasForUpdate(_parameter)\">");
    ...
    public static String exampleCheck(Class<?> entityClass) {
        StringBuilder sql = new StringBuilder();
        sql.append(
            "<bind name=\"checkExampleEntityClass\" value=\"@com.ring.welkin.common.persistence.mybatis.mapper.example.OGNL@checkExampleEntityClass(_parameter, '");
