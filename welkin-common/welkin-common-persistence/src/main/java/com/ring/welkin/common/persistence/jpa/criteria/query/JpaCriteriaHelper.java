package com.ring.welkin.common.persistence.jpa.criteria.query;
...
public class JpaCriteriaHelper<T> {
    ...
    private class WhereEntry {
        ...
        public WhereEntry(List<String> fieldNames, ComparatorOperator comparatorOperator, Object valueIni,
                          Object valueEnd, LogicalOperator logicalOperator) {
    ...
    /**
     * Define nome do campo a ser ordenado, assim como a direção em que deve ser
     * ordenado
     */
     ...
     *
     * @param fieldName Nome da propriedade é necessário que valor complemente
     *                  {@link Comparable})</b>
     * @param value     Valor
     * @return objeto de consulta
     */
     ...
     * Inclui uma clausula WHERE
     *
     * @param fieldNames Nome da propriedade é necessário que valor complemente
     *                   {@link Comparable})</b>
     * @param value      Valor
     * @return objeto de consulta
     */
     ...
     * @param valueIni   Valor inicial <b>(necessita implementar
     *                   {@link Comparable})</b>
    ...
    public JpaCriteriaHelper<T> where(String fieldName, ComparatorOperator comparator, Comparable valueIni,
                                      Comparable valueEnd) {
     ...
     */
    @SuppressWarnings({"rawtypes"}) // TODO: tentar resolver este warning
    public JpaCriteriaHelper<T> and(String fieldName, ComparatorOperator comparator, Comparable valueIni,
                                    Comparable valueEnd) {
     ...
     * Inclui uma clausula WHERE após um operador AND
     *
     * @param fieldNames Nome das propriedades é necessário que valor complemente
     *                   {@link Comparable})</b>
     * @param value      Valor
     * @return objeto de consulta
     */
     ...
     */
    @SuppressWarnings({"rawtypes"}) // TODO: tentar resolver este warning
    public JpaCriteriaHelper<T> or(String fieldName, ComparatorOperator comparator, Comparable valueIni,
                                   Comparable valueEnd) {
    ...
    /**
     * Inclui clausula GROUP BY
     *
     * @param fieldName Nome da propriedade
     * @return objeto de consulta
     */
    public JpaCriteriaHelper<T> groupBy(String fieldName) {
        demandsOperation(SqlOperation.SELECT);
        groups.add(new GroupBy(fieldName));
        return this;
    }
    ...
    private <E, Y> void groupBy(CriteriaQuery<E> criteriaQuery, Root<Y> root) {
        if (!groups.isEmpty()) {
            for (GroupBy groupField : groups) {
                criteriaQuery.multiselect(criteriaBuilder.count(root.get(Idable.ID)), root.get(groupField.fieldName));
                criteriaQuery.groupBy(root.get(groupField.fieldName));
            }
        }
    }
    ...
    /**
     * Define a pagina que serah retornada <b>Se a página for informada, ativa a
     * paginação de resultados</b> Por padrão, não será efetuada paginação dos
     * resultados
     *
     ...
     *
     * @return O primeiro objeto retornado da consulta ou <b>null</b> se a
     * consulta não retornar resultados
     */
     ...
     * @return
     */
    @SuppressWarnings({"rawtypes", "unchecked"}) // TODO: tentar retirar estes
    // warnings
    public int execute() {
        demandsOperation(SqlOperation.UPDATE);
    ...
    private void addTowhere(List<String> fieldNames, ComparatorOperator comparator, Object valueIni, Object valueEnd,
        ...
        if ((comparator.equals(ComparatorOperator.GREATER_THAN) || comparator.equals(ComparatorOperator.LESS_THAN))
            ...
            throw new RuntimeException("Para os tipos de operador " + ComparatorOperator.GREATER_THAN + " e "
                + ComparatorOperator.LESS_THAN + " é necessário que o objeto de valor implemente "
        ...
        if (valueEnd != null && !comparator.equals(ComparatorOperator.BETWEEN)) {
            throw new RuntimeException(
                "Quando informados dois valores, é obrigatório o uso de " + ComparatorOperator.BETWEEN);
        }
    ...
    @SuppressWarnings({"unchecked", "rawtypes"}) // TODO: tentar retirar estes
    // warnings
        ...
        for (WhereEntry whereEntry : wheres) {
            ...
            switch (whereEntry.comparatorOperator) {
                ...
                case LIKE:
                    ...
                    break;
                case NOT_LIKE:
                    predicate = criteriaBuilder.notLike(path, whereEntry.valueIni.toString());
                    break;
                case LIKE_IGNORE_CASE:
                    predicate = criteriaBuilder.like(criteriaBuilder.upper(path),
                        whereEntry.valueIni.toString().toUpperCase());
                    break;
                case IN:
                    predicate = path.in((Collection) whereEntry.valueIni);
                    break;
                case BETWEEN:
                    predicate = criteriaBuilder.between(path, (Comparable) whereEntry.valueIni,
                        (Comparable) whereEntry.valueEnd);
                    break;
                default:
                    throw new RuntimeException(
                        "Tipo de operador de comparação não conhecido: " + whereEntry.comparatorOperator);
            }
            ...
            if (predMaster == null) {
                ...
                if (whereEntry.logicalOperator != null) {
                    switch (whereEntry.logicalOperator) {
                        ...
                        default:
                            throw new RuntimeException(
                                "Tipo de operador lógico não conhecido: " + whereEntry.comparatorOperator);
                    }
                }
            }
    ...
    /**
     * Obtem as columns especificadas na classe tupleClazz E a partir da
     * entityClass T.
     *
     * <p>
     * A classe tupleClazz precisa possuir um construtor que respeite a ordem
     * das columns informadas.
     *
    ...
    /**
     * Confere se a operação atual do objeto é a operação esperada. Lança uma
     * exceção se a operação atual não for igual à esperada.
     *
