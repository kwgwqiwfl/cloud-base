...
public class Example implements IDynamicTableName {
	...
	// 直接指定查询字段，优先级最高
	protected Set<String> originalColumns;
	...
	// 自动忽略查询@ColumnQueryIgnore注解的字段对应列
	...
	private Example(Builder builder) {
		...
		this.igoneColumn = builder.igoneColumn;
			...
			for (String property : properties) {
				if (propertyMap.containsKey(property)) {
					...
					throw new MapperException(
						"Class " + entityClass.getSimpleName() + " does not contain an attribute \'" + property
							+ "\'，Or the attribute is annotated by @Transient.");
				}
			}
		}
		return this;
	...
	public Example originalColumns(String... originalColumns) {
		if (originalColumns != null && originalColumns.length > 0) {
			if (this.originalColumns == null) {
				this.originalColumns = new LinkedHashSet<String>();
			}
			this.originalColumns.addAll(Sets.newHashSet(originalColumns));
		}
		return this;
	}
	...
	public static class Criteria extends FieldItem {
		...
		private Class<?> javaType(String property) {
			if (propertyMap.containsKey(property)) {
				...
				throw new MapperException("Current entity class does not contain a property named '" + property + "'.");
			} else {
				return null;
			}
		}
			...
			try {
				...
				if (javaType != null && javaType.isAssignableFrom(Date.class)) {
					if (value instanceof String) {
                        value = DateTimeFormatterUtil.smartParseToDate((String) value);
					} else if (value instanceof Long) {
						value = new Date((long) value);
					}
				}
			} catch (Exception e) {
				throw new RuntimeException(String.format("Can not eval value [%s] for property [%s]", value, property));
			}
			return value;
		}
	...
	public static class Builder {
		...
		public Builder originalColumns(Set<String> originalColumns) {
			setOriginalColumns(originalColumns);
			return this;
		}

		public Builder igoneColumn() {
			return setIgoneColumn(true);
		}
			...
			if (properties != null && properties.length > 0) {
				...
				for (String property : properties) {
					if (this.propertyMap.containsKey(property)) {
						...
						throw new MapperException(
							"Current entity class does not contain an attribute named " + property);
					}
				}
			}
			return this;
		...
		public Builder setIgoneColumn(boolean igoneColumn) {
	...
	public Set<String> getSelectColumns() {
		if (ICollections.hasElements(originalColumns)) {
