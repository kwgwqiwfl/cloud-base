 ...
 */
public interface TreeCodeableService<C extends Serializable, T extends TreeCodeable<C, T>> {
    ...
    default List<T> findRoots(List<T> allList) {
        if (ICollections.hasElements(allList))
            return allList.stream().filter(t -> t.isRoot()).sorted().collect(Collectors.toList());
        return allList;
    }
    ...
    default List<T> findNotRoots(List<T> allList) {
        if (ICollections.hasElements(allList))
            return allList.stream().filter(t -> !t.isRoot()).collect(Collectors.toList());
        return Lists.newArrayList();
    }
