...
public class PasswordGenerator {
    ...
    private List<CharactorType> allowedTypes;// 允许的字符类型

    public PasswordGenerator(int passwordLength, int minVariousType, CharactorType... allowedCharactorTypes) {
        this.allowedTypes = Arrays
            .asList(ArrayUtils.isNotEmpty(allowedCharactorTypes) ? allowedCharactorTypes : CharactorType.values());
        if (minVariousType > allowedTypes.size())
            minVariousType = allowedTypes.size();
    ...
    public String generateRandomPassword() {
        ...
        List<CharactorType> takeTypes = new ArrayList<>(allowedTypes);
        int typeCount = 0;
        while (pwCharsIndex.size() > 0) {
            ...
            if (typeCount < minVariousType) {// 生成不同种类字符
                ...
                c = generateCharacter(allowedTypes.get(random.nextInt(allowedTypes.size())));
            }
    ...
    public static void main(String[] args) {
        PasswordGenerator passwordGenerator = new PasswordGenerator(10, 3, CharactorType.LOWERCASE,
            CharactorType.UPPERCASE, CharactorType.NUMBER);
