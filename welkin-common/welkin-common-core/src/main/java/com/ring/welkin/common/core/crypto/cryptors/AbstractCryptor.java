...
public abstract class AbstractCryptor implements Cryptor {
	...
	public AbstractCryptor(String prefix, String suffix, String prefix1, String suffix1) {
	...
	public boolean isEncrypted(String ciphertext) {
		...
		return (trimmedValue.startsWith(prefix) && trimmedValue.endsWith(suffix)) || isEncrypted1(trimmedValue);
	}

	protected boolean isEncrypted1(String ciphertext) {
		if (StringUtils.isNotBlank(prefix1) && StringUtils.isNotBlank(suffix1)) {
			final String trimmedValue = ciphertext.trim();
			return (trimmedValue.startsWith(prefix1) && trimmedValue.endsWith(suffix1));
		}
		return false;
	}
	...
	public String unwrapEncryptedValue(String ciphertext) {
		if (isEncrypted1(ciphertext)) {
			return ciphertext.substring(prefix1.length(), (ciphertext.length() - suffix1.length()));
		}
