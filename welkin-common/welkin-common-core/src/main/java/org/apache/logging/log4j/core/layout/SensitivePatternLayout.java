...
/**
 * 自定义log4j2 layout， 扩展自PatternLayout
 */
...
public final class SensitivePatternLayout extends AbstractStringLayout {
	 ...
	 * @param footer
	 */
	private SensitivePatternLayout(final Configuration config, final RegexReplaces regexReplaces, final String pattern,
	...
	/**
	 * log4j2 拷贝代码 Create a pattern layout.
	 *
	 ...
	 *                              always written even if the pattern contains no
	 *                              exception tokens.
	 * @param noConsoleNoAnsi       If {@code "true"} (default is false) and
	 *                              {@link System#console()} is null, do not output
	 *                              ANSI escape codes
	 * @param header                The footer to place at the top of the document,
	 *                              once.
	 * @param footer                The footer to place at the bottom of the
	 *                              document, once.
	...
	public static SensitivePatternLayout createLayout(
			...
			@PluginElement("RegexReplaces") final RegexReplaces regexReplaces,
		...
		return newBuilder().withPattern(pattern).withConfiguration(config).withRegexReplaces(regexReplaces)
	 ...
	 * using UTF-8, the default conversion pattern, exceptions being written, and
	...
	public static class Builder implements org.apache.logging.log4j.core.util.Builder<SensitivePatternLayout> {
		...
		private RegexReplaces regexReplaces = null;
		...
		public Builder withRegexReplaces(final RegexReplaces regexReplaces) {
		...
		public SensitivePatternLayout build() {
			...
			return new SensitivePatternLayout(configuration, regexReplaces, pattern, charset, alwaysWriteExceptions,
