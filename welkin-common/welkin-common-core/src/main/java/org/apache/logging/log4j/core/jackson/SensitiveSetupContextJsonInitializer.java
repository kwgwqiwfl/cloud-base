...
public class SensitiveSetupContextJsonInitializer {
    ...
    void setupModule(final SetupContext context, final boolean includeStacktrace, final boolean stacktraceAsString) {
        ...
        // Log4j API classes: we do not want to edit those with Jackson annotations
        // because the API module should not depend on Jackson.
        ...
        context.setMixInAnnotations(LogEvent.class, SensitiveLogEventJsonMixIn.class); // different ThreadContext handling
        // Log4j Core classes: we do not want to bring in Jackson at runtime if we do
        // not have to.
