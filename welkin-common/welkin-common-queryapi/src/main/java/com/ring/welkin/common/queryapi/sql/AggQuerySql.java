package com.ring.welkin.common.queryapi.sql;

import com.ring.welkin.common.queryapi.query.aggregate.AggField;
...
public class AggQuerySql extends AbstractQuerySql<AggQuery> {
    ...
    protected void build(AggQuery query) {
        if (valid(query)) {
            ...
            // having 条件
            buf.append(having(query.getHavingFieldGroup(),query.getAggFields()));
    ...
    protected String having(FieldGroup havingFieldGroup,LinkedHashSet<AggField> aggFields) {
        StringBuffer buf = new StringBuffer();
